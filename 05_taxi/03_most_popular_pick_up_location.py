...
''' 
python 03_most_popular_pick_up_location.py test.txt
python 03_most_popular_pick_up_location.py yellow_tripdata_2016-01.csv > pick_up.csv
'''
...
class MRTaxi(MRJob):
    ...
    def mapper_get_keys(self,key,value):
        yield None,(value,key) #retrun number and location

    def reducer_get_sorted(self,key,values):
        self.results=[]
        for value in values:
            self.results.append((key,value))
        yield None, sorted(self.results,reverse=True)[:10] #first 10 elem
